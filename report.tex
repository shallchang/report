...
The normal order reduction $M\xrightarrow{no} N$ continually applies te rule for $\beta$-reduction on the redex in leftmost outermost position until no more $\beta$-reduction can be performed. At that point, the resulting term is in normal form. When reducing an application $(MN)$, the function term $M$ must be reduced using call-by-name. Since the strategy is outermost, when $M$ is reduced to an abstraction $(\lambda x.P)$, then the redex $((\lambda x.P)N)$ must be reduced before redexes in P.

\begin{equation*}
x \xrightarrow{no}x
\end{equation*}
\begin{equation*}
\frac{M \xrightarrow{no}N}{(\lambda x.M) \xrightarrow{no} (\lambda x.N)}
\end{equation*}
\begin{equation*}
\frac{M \xrightarrow{bn} (\lambda x.P)\ \ \ \ \ \ \ \ \ P[N/x] \xrightarrow{no} Q}{(MN) \xrightarrow{no} Q}
\end{equation*}
\begin{equation*}
\frac{M \xrightarrow{bn} P\neq(\lambda x.R)\ \ \ \ \ \ \ \ \ P[N/x] \xrightarrow{no} Q}{(MN) \xrightarrow{no} Q}
\end{equation*}
